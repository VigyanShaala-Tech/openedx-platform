...
def create_account_with_params(request, params):  # pylint: disable=too-many-statements
    ...
    * Duplicate email raises a ValidationError (rather than the expected
    ...
    with outer_atomic():
        ...
        # Only auto-login for third-party signup (Google,) By Yagnesh
        if third_party_provider or running_pipeline:
...
class RegistrationView(APIView):
    ...
    def post(self, request):
        ...
        redirect_to, root_url = get_next_url_for_login_page(request, include_host=True)
        redirect_url = get_redirect_url_with_host(root_url, redirect_to)
        authenticated_user = {'username': user.username, 'full_name': user.profile.name, 'user_id': user.id, 'is_third_party_user': bool(pipeline.running(request))}
        response = self._create_response(
            request, {'authenticated_user': authenticated_user}, status_code=200, redirect_url=redirect_url
        )
        # Modified by Yagnesh (Only set cookies if user is active (Google users))
        if user.is_active:
